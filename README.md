...
## v0.2 consoles and capabilities

The new console is aligned across all four platforms: dark navigation, workspace switching, overview metrics, searchable records, detail drawers and private artifact previews. Each Java repository includes its own independently buildable `frontend/`; AI-SDLC owns the main workbench.

- **Automation:** Web and API case library, immutable revisions, Workflow compilation, actual execution history, assertions, HTTP details and browser screenshots.
- **AI-SDLC:** DD-style AI CR, Markdown test-case generation and Markdown-to-Workflow compilation; review findings retain evidence and missing-context states.
- **Gateways:** model provider/status/playground/request diagnostics; sandbox profiles/execution details/logs/screenshots.
- **Storage:** Supabase Storage only. No additional S3/MinIO service or credentials required.
- **Demo data:** twelve editable, executable sample cases. Seeded cases are labeled; execution outcomes and metrics are generated by actual runs.

Read [the precise v0.2 scope](docs/v0.2-scope.md). Model mode remains explicitly visible; mock capability output is not a real AI-analysis result.
...
v0.2 supports Web and HTTP API workflows with an isolated synthetic application by default and operator-registered targets. It is not an autonomous browser exploration agent, full scheduling platform or arbitrary code runner. Git tasks snapshot registered repositories; they do not execute code from a cloned repository or push commits. Generated test behavior stays in the constrained Workflow contract.
...
`contracts/workflow-v1.schema.json` describes the accepted shape. Java and Runner validators enforce the operational constraints (including a real nonblank assertion). Version 1 operations are `fill(title)`, `click(add)` and `assertText(items)`. Version 2 adds registered targets, Web locators/actions and HTTP/JSON assertions; see `contracts/workflow-v2.md`. Values are serialized as data, never inserted as raw JavaScript.
...
docker build -t ai-sdlc-runner:0.2.0 .
```
