...
public class AutomationApi {
 ...
 @PostMapping("/runs") public ResponseEntity<?> submit(@RequestBody RunRequest req,@RequestHeader("Idempotency-Key")String key,Authentication auth){var task=jobs.submit(Access.owner(auth),key,Map.of("kind","automation","title",Objects.toString(req.title(),"自动化执行"),"request",req.validate(git)));return ResponseEntity.status(task.created()?202:200).body(jobs.detail(task.job()));}
 ...
 @GetMapping("/runs/{id}") public Object get(@PathVariable String id,Authentication auth){return jobs.detail(jobs.get(id,Access.owner(auth)));}
 ...
 @PostMapping("/compile") public Object compile(@RequestBody Workflow w){w.validate();String spec=w.compile();return Map.of("workflow",w,"workflowHash",w.hash(),"compilerVersion",w.compilerVersion(),"spec",spec,"specHash",Json.hash(spec));}
 ...
 @GetMapping("/repository-tasks/{id}") public Object gitResult(@PathVariable String id,Authentication auth){return jobs.detail(jobs.get(id,Access.owner(auth)));}
}
