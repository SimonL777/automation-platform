...
public class Artifacts {
 private final String mode,url,key,bucket;private final Path dir;private final HttpClient http=HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(10)).build();
 public Artifacts(@Value("${app.artifact-mode}")String mode,@Value("${app.artifact-dir}")String dir,@Value("${app.supabase-url}")String url,@Value("${app.supabase-service-key}")String key,@Value("${app.storage-bucket}")String bucket){
  this.mode=mode;this.dir=Path.of(dir).toAbsolutePath().normalize();this.url=url.replaceAll("/$","");this.key=key;this.bucket=bucket;
 ...
 public void put(String owner,String id,String name,byte[] bytes){if(bytes.length>8_000_000)throw new ApiFailure(413,"ARTIFACT_TOO_LARGE");String p=path(owner,id,name);
  try{if(mode.equals("local")){Path file=dir.resolve(p);Files.createDirectories(file.getParent());Files.write(file,bytes);return;}
   ...
   int status=http.send(req,HttpResponse.BodyHandlers.discarding()).statusCode();if(status/100!=2)throw new ApiFailure(502,"STORAGE_UPLOAD_FAILED");
 ...
 public byte[] get(String owner,String id,String name){String p=path(owner,id,name);try{
  ...
  var res=http.send(req,HttpResponse.BodyHandlers.ofByteArray());if(res.statusCode()==404)throw new ApiFailure(404,"ARTIFACT_NOT_FOUND");if(res.statusCode()/100!=2)throw new ApiFailure(502,"STORAGE_READ_FAILED");if(res.body().length>8_000_000)throw new ApiFailure(502,"ARTIFACT_TOO_LARGE");return res.body();
 }catch(ApiFailure e){throw e;}catch(Exception e){if(e instanceof InterruptedException)Thread.currentThread().interrupt();throw new ApiFailure(502,"STORAGE_UNAVAILABLE");}}
}
